...
export default function useProgram() {
    ...
    const addOperation = async (operation: BaseOperationSchema, previousId: String) => {
        ...
        if(operation.type == OperationType.Loop) {
            const _start = {...(await getDefaultOperation(OperationType.Start, 0)), parent: String(operation.id), name: "Inicio", id: operation.id + 1}
            const _end = {...(await getDefaultOperation(OperationType.End, 1)), parent: String(operation.id), name: "Fin", id: operation.id + 2}
            ...
            const _startYes = {...(await getDefaultOperation(OperationType.Start, 0)), parent: String(operation.id + '_yes'), name: "Inicio", id: operation.id + 1}
            const _endYes = {...(await getDefaultOperation(OperationType.End, 1)), parent: String(operation.id + '_yes'), name: "Fin", id: operation.id + 2}
            const _startNo = {...(await getDefaultOperation(OperationType.Start, 0)), parent: String(operation.id + '_no'), name: "Inicio", id: operation.id + 3}
            const _endNo = {...(await getDefaultOperation(OperationType.End, 1)), parent: String(operation.id + '_no'), name: "Fin", id: operation.id + 4}
