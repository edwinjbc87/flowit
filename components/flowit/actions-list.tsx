...
export default function ActionsList({onSelected}:{onSelected:(node:NodeType)=>void}) {
    const intl = useIntl();
    ...
    return (
        <div>
            {actions.filter(ac => ac.type != NodeType.Start && ac.type != NodeType.End && ac.type != NodeType.Yes && ac.type != NodeType.No).map(ac => (
                <button onClick={()=>onSelected(ac.type)} key={ac.name} className="flex items-center p-2 rounded-md border border-gray-300 mb-1 w-full hover:bg-gray-100">
